...

{
    ...
    "category": "section",
    "text": "In some cases one needs to have access to the value of an axis, for example when one wants to calculate a spatial Aggregation, the latitudes are important to determine grid cell weights. To do this, one can pass a cube axis to mapCube as if it was a cube having only one dimension.function spatialAggregation{T}(xout::Array{T,0}, xin::Matrix, latitudes::AbstractVector)\n  #code goes here\nend\ngetLatAxis(cube,pargs)=getAxis(LatAxis, cube[1])\n\ninAxes=((LonAxis, LatAxis), (getLatAxis,))\noutAxes=()\nregisterDATFunction(spatialAggregation, inAxes, outAxes, inmissing=(:dataarray,:nan), outmissing=:dataarray)Here, the function will operate on a lon x lat matrix and one has access to the latitude values inside the function. We c"
},

{
    ...
    "category": "section",
    "text": "For some calculations the output axis does not equal any of the input axis, but has to be generated before the cube calculation starts."
},
